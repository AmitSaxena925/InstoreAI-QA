package com.capillary.VisitorMatrix.qa.framework.base;
...
import com.capillary.VisitorMatrix.qa.framework.element.HTMAlert;
...
public abstract class BasePage {
	...
	public void assertContainsText(String text) {
		Reporter.log("<b>Assert that applicationPages contains text</b> <br> Page | Text => " + pageName + " | " + text
				+ "<br>");
		Assertions.assertThat(DriverFactory.getDriver().findElement(By.tagName("body")).getText())
				.containsIgnoringCase(text);
	}

	public void assertDoesNotContainText(String text) {
		Reporter.log("<b>Assert that applicationPages doesn't contain text</b> <br> Page | Text => " + pageName + " | "
				+ text + "<br>");
	...
	public void assertContainsTitle(String title) {
		Reporter.log("<b>Assert that applicationPages contains title</b> <br> Page | Text => " + pageName + " | "
				+ title + "<br>");
	...
	public void waitUntilLoads() {
		...
		try {
			...
			// link results in a
			// applicationPages load & we have a waitForPageLoad after click then if the
			// document hasn't started to load, it will
			...
			// Do nothing since we don't want to fail a test if the applicationPages hasn't
			// loaded completely
		} finally {
		...
		try {
			...
			if (timeOutInMilliSeconds > 0) {
				wait.withTimeout(timeOutInMilliSeconds, TimeUnit.MILLISECONDS)
						.until(CustomExpectedConditions.jQueryAJAXCallsHaveCompleted());
			}
			pause(1);
